...
(function () {
	...
	rectPath =
		R._rectPath = function (x, y, w, h, r) {
		if (r)
	...
	function cacher(f, scope, postprocessor) {
		function newf() {
			...
			postprocessor(cache[args]) : cache[args]
		}
		return newf
	}
	...
	function catmullRom2bezier(crp, z) {
		...
		for (var i = 0, iLen = crp.length; iLen - 2 * !z > i; i += 2) {
			...
			d.push(["C", (-p[0].x +
						6 * p[1].x + p[2].x) / 6, (-p[0].y + 6 * p[1].y + p[2].y) / 6, (p[1].x + 6 * p[2].x - p[3].x) / 6, (p[1].y + 6 * p[2].y - p[3].y) / 6, p[2].x, p[2].y])
		}
		return d
	}
	...
	R.isBBoxIntersect = function (bbox1,
		bbox2) {
	...
	function getTatLen(x1, y1, x2, y2, x3,
		y3, x4, y4, ll) {
	...
	findDotAtSegment = function (p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y, t) {
		...
		return {
			x : pow(t1, 3) *
			p1x + pow(t1, 2) * 3 * t * c1x + t1 * 3 * t * t * c2x + pow(t, 3) * p2x,
	...
	curveDim = cacher(function (p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y) {
			...
			if (t2 > 0 && t2 < 1) {
				dot =
					findDotAtSegment(p1x, p1y, c1x, c1y, c2x, c2y, p2x, p2y, t2);
				x.push(dot.x);
				y.push(dot.y)
			}
	...
	path2curve = R._path2curve = cacher(function (path, path2) {
			...
			for (var i = 0, ii = mmax(p.length, p2 && p2.length || 0); i < ii; i++) {
				...
				attrs2.bx = p2 && (toFloat(seg2[seg2len -
								4]) || attrs2.x);
	...
	(function (matrixproto) {
		matrixproto.add = function (a, b, c, d, e, f) {
			...
			if (a && a instanceof Matrix)
				matrix = [[a.a, a.c, a.e], [a.b, a.d, a.f],
					[0, 0, 1]];
			for (x = 0; x < 3; x++)
				for (y = 0; y < 3; y++) {
					res = 0;
		...
		matrixproto.get = function (i) {
			return +this[Str.fromCharCode(97 +
					i)].toFixed(4)
		};
	...
	if (navigator.vendor == "Apple Computer, Inc." && (version && version[1] < 4 || navigator.platform.slice(0,
				2) == "iP") || navigator.vendor == "Google Inc." && version && version[1] < 8)
	...
	dragMove = function (e) {
		...
		while (j--) {
			...
			eve("raphael.drag.move." +
				dragi.el.id, dragi.move_scope || dragi.el, x - dragi.el._drag.x, y - dragi.el._drag.y, x, y, e)
		}
	},
	dragUp = function (e) {
	...
	paperproto.ellipse = function (x, y, rx, ry) {
		var out =
			R._engine.ellipse(this, x || 0, y || 0, rx || 0, ry || 0);
		this.__set__ && this.__set__.push(out);
		return out
	};
	...
	paperproto.setSize = function (width,
		height) {
	...
	var getOffset = function (elem) {
		...
		top = box.top + (g.win.pageYOffset || docElem.scrollTop || body.scrollTop) -
			clientTop,
	...
	animation = function () {
		...
		for (; l < animationElements.length; l++) {
			...
			key;
			if (e.initstatus) {
				time = (e.initstatus * e.anim.top - e.prev) / (e.percent - e.prev) *
				ms;
					...
					if (availableAnimAttrs[has](attr) || element.paper.customAttributes[has](attr)) {
						...
						case "transform":
							...
							if (eq) {
								...
								diff[attr] = [(to2.matrix.a - m.a) / ms, (to2.matrix.b - m.b) / ms, (to2.matrix.c - m.c) / ms, (to2.matrix.d - m.d) / ms, (to2.matrix.e - m.e) / ms, (to2.matrix.f -
										m.f) / ms]
							}
							break;
						case "csv":
	...
	elproto.resume = function (anim) {
		for (var i = 0; i < animationElements.length; i++)
			if (animationElements[i].el.id ==
				this.id && (!anim || animationElements[i].anim == anim)) {
				var e = animationElements[i];
	...
	function stopAnimation(paper) {
		for (var i =
				0; i < animationElements.length; i++)
	...
	setproto.forEach = function (callback, thisArg) {
		for (var i = 0, ii = this.items.length; i < ii; i++)
			if (callback.call(thisArg, this.items[i],
					i) === false)
				return this;
		return this
	};
	...
	paperproto.getFont = function (family, weight,
		style, stretch) {
		...
		if (font)
			for (var i = 0, ii = font.length; i < ii; i++) {
				thefont = font[i];
				if (thefont.face["font-weight"] == weight && (thefont.face["font-style"] ==
...
window.Raphael.svg && function (R) {
			...
			if (params[has](att)) {
				...
				case "path":
					if (o.type == "path") {
						...
						if (o._.arrows) {
							"startString" in
							o._.arrows && addArrow(o, o._.arrows.startString);
				...
				case "fill":
					...
					if (!clr.error) {
						...
						!R.is(attrs["fill-opacity"], "undefined") && R.is(params["fill-opacity"], "undefined") &&
						$(node, {
				...
				case "stroke":
					...
					break;
				case "gradient":
					(o.type == "circle" || o.type == "ellipse" || Str(value).charAt() !=
						"r") && addGradientFill(o, value);
					break;
				case "opacity":
	...
	elproto.attr = function (name, value) {
		...
		if (name == null) {
			...
			for (var a in this.attrs)
				if (this.attrs[has](a))
					res[a] = this.attrs[a];
			res.gradient &&
			res.fill == "none" && (res.fill = res.gradient) && delete res.gradient;
		...
		for (key in this.paper.customAttributes)
			if (this.paper.customAttributes[has](key) && params[has](key) && R.is(this.paper.customAttributes[key],
					"function")) {
	...
	elproto.insertAfter = function (element) {
		if (this.removed)
			return this;
		var node = element.node || element[element.length -
				1].node;
		if (node.nextSibling)
...
window.Raphael.vml && function (R) {
	...
	setFillAndStroke = function (o, params) {
		...
		isOval = ovalTypes[o.type] && (a.cx != params.cx || a.cy != params.cy || a.r != params.r ||
				a.rx != params.rx || a.ry != params.ry),
		res = o;
		...
		if (params.opacity != null || params["stroke-width"] != null || params.fill != null ||
			params.src != null || params.stroke != null || params["stroke-width"] != null || params["stroke-opacity"] != null || params["fill-opacity"] != null || params["stroke-dasharray"] != null || params["stroke-miterlimit"] != null || params["stroke-linejoin"] != null || params["stroke-linecap"] != null) {
			...
			opacity = ((+a["stroke-opacity"] +
					1 || 2) - 1) * ((+a.opacity + 1 || 2) - 1) * ((+strokeColor.o + 1 || 2) - 1);
			...
			params["stroke-linecap"] && (stroke.endcap = params["stroke-linecap"] ==
					"butt" ? "flat" : params["stroke-linecap"] == "square" ? "square" : "round");
	...
	addGradientFill = function (o, gradient, fill) {
		...
		if (dots.length) {
			...
			var clrs = [];
			for (var i = 0, ii = dots.length; i < ii; i++)
				dots[i].offset && clrs.push(dots[i].offset +
					S + dots[i].color);
		...
		if (params) {
			for (key in this.paper.customAttributes)
				if (this.paper.customAttributes[has](key) &&
					params[has](key) && R.is(this.paper.customAttributes[key], "function")) {
					var par = this.paper.customAttributes[key].apply(this, [].concat(params[key]));
					this.attrs[key] = params[key];
					for (var subkey in par)
						if (par[has](subkey))
	...
	R._engine.create = function () {
		var con = R._getContainer.apply(0,
				arguments),
...
Beetny.EQ2AA.Rendering.RaphaelRenderer = Class.extend({
		...
		updateAAColor : function (aa, components) {
			...
			if (aa.hasParent())
				components.parentLine.attr("stroke", aa.level > 0 ? "#339933" : "#666666")
		},
		...
		renderAAComponents : function (canvas, aa) {
			...
			function linesToParent() {
				...
				if (aa.parent_id > -1) {
					...
					path = canvas.path("M" + x1 + " " + y1 + "L" + x2 + " " + y2).attr("stroke-width", 3).toBack()
				}
				return path
			}
		}
...
Beetny.EQ2AA.Tooltips = {
	...
	createOrUpdateAATooltips : function (aa, tooltips) {
		...
		function createTooltip() {
			var template = '<div class="aa-tooltip">' + '<div class="header">' +
				'<span class="name"></span>' + '<span class="rank"></span>' + "<br>" + '<span class="subclass"></span>' + '<span class="cost"></span>' + "<br>" + '<span class="prereqs"></span>' + "</div>" + '<div class="description"></div>' + '<div class="effects">' + "<span></span>" + "<div></div>" + "</div>" + "</div>";
			return $(template)
		}
		function fillTooltipData(jTooltip, level) {
			...
			if (aa.effects.length > 0) {
				...
				effects = aa.getEffectForLevel(level === 0 ? level + 1 :
						level)
			}
			var costText = "";
			...
			function buildPrereqsList() {
				...
				function makeTreeRatioItem() {
					if (aa.tree.x_y_ratio > 0) {
						var text = "Every {Ratio} points in {x} unlocks 1 point in {y}".replace("{Ratio}", aa.tree.x_y_ratio).replace("{x}",
								aa.tree.x_subclass).replace("{y}", aa.tree.y_subclass);
				...
				function makePrereqItem(prereqName) {
					...
					if (aa.prereqs[prereqName] > 0) {
						...
						var text = prereqText[prereqName].replace("{0}", replacements[0]).replace("{1}",
								replacements[1]);
...
$(function () {
	...
	Beetny.EQ2AA.Main.apply(Beetny.EQ2AA,
		[classLoader, renderer, importer, hash])
});
